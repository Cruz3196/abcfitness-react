...
import { MessageCircle, X, Send, ShoppingBag, Calendar, Dumbbell, HelpCircle, Search, Trash2 } from "lucide-react";
...
const ChatAssistant = () => {
  ...
  const quickActions = [
    {
      ...
      label: "Shop Clothes",
      icon: ShoppingBag,
      response: "Great choice! We have a wide selection of fitness apparel. Let me take you to our store where you can browse workout clothes, activewear, and accessories.",
    ...
    },
    {
      ...
      label: "Book a Class",
      icon: Calendar,
      response: "Awesome! We offer various fitness classes including yoga, HIIT, strength training, and more. Let me show you our available classes.",
    ...
    },
    {
      ...
      icon: Dumbbell,
      response: "Looking for personalized training? Our certified trainers can help you reach your fitness goals. Let me show you our team.",
    ...
    },
    {
      ...
      icon: Search,
      response: "Sure! What product are you looking for? Type a keyword like 'shirt', 'shoes', 'yoga mat', etc.",
      action: null,
    },
  ];
  ...
  const formatProductResults = (matchedProducts) => {
    ...
    let response = `I found ${matchedProducts.length} product${matchedProducts.length > 1 ? 's' : ''} for you:\n\n`;
  ...
  const handleSendMessage = (e) => {
    ...
    if (userMessage.includes("search") || userMessage.includes("find") || userMessage.includes("looking for")) {
      const searchTerms = userMessage.replace(/search|find|looking for|product|products/gi, "").trim();
      if (searchTerms.length > 2) {
        const results = searchProducts(searchTerms);
        botResponse = formatProductResults(results);
      } else {
        botResponse = "What would you like to search for? Tell me the product name or type (e.g., 'search yoga mat' or 'find shirts').";
      }
    }
    // Check for recommendations
    else if (userMessage.includes("recommend") || userMessage.includes("suggest") || userMessage.includes("popular")) {
      if (userMessage.includes("class") || userMessage.includes("workout") || userMessage.includes("exercise")) {
    ...
    else if (userMessage.includes("cloth") || userMessage.includes("shop") || userMessage.includes("buy") || userMessage.includes("apparel") || userMessage.includes("wear") || userMessage.includes("store")) {
      botResponse = "I'd be happy to help you find some great fitness gear! Let me take you to our store.";
      navigationAction = () => navigate("/store");
    } else if (userMessage.includes("class") || userMessage.includes("book") || userMessage.includes("schedule") || userMessage.includes("yoga") || userMessage.includes("hiit")) {
      botResponse = "Looking to book a class? We have plenty of options! Let me show you what's available.";
      navigationAction = () => navigate("/classes");
    } else if (userMessage.includes("trainer") || userMessage.includes("coach") || userMessage.includes("personal") || userMessage.includes("training")) {
      botResponse = "Our trainers are top-notch! Let me introduce you to our team.";
    ...
    } else if (userMessage.includes("order") || userMessage.includes("purchase") || userMessage.includes("bought")) {
      if (user) {
        botResponse = "To check your orders, I can take you to your profile page where all your order history is saved.";
        navigationAction = () => navigate("/profile");
      } else {
        botResponse = "Please log in to view your order history. Would you like me to take you to the login page?";
      }
  ...
  return (
    <>
      ...
      >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs opacity-80">
                  {user ? `Chatting as ${user.username}` : "Always here to help"}
                </p>
              </div>
            </div>
            <button
        ...
        {/* Input */}
        <form onSubmit={handleSendMessage} className="p-4 pt-2 border-t border-base-300">
