...
export const userStore = create((set, get) => ({
  ...
  checkAuthStatus: async (force = false) => {
    ...
    // Use force=true to bypass this check (e.g., after profile updates)
    if (!force && currentState.user && currentState.isAuthenticated) {
  ...
  resetPassword: async (token, password) => {
    set({ isLoading: true });
    try {
      const { data } = await axios.put(`/user/resetPassword/${token}`, {
        password,
      });
      toast.success(data.message);
      return true;
  ...
  createTrainerProfile: async (trainerData) => {
    ...
    try {
      ...
      // Force refresh to get updated hasTrainerProfile status
