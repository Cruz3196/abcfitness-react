...
export const getAllOrders = async (req, res) => {
  try {
    ...
    console.log(
      "Error in fetching all orders in admin controller",
      error.message
    );
...
export const viewAllUsers = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: error.message });
    console.log(
      "Error in viewing all users in admin controller",
      error.message
    );
  }
};
...
export const pendingTrainerProfiles = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: error.message });
    console.log(
      "Error in viewing pending trainer profiles in admin controller",
      error.message
    );
  }
};
...
export const viewAllTrainers = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: error.message });
    console.log(
      "Error in viewing all trainers in admin controller",
      error.message
    );
  }
};
...
export const viewClassInsights = async (req, res) => {
  try {
    ...
    console.log(
      "Error in viewing class insights in admin controller",
      error.message
    );
    res.status(500).json({ message: error.message });
  }
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const bookingRevenueStats = await Booking.aggregate([
      ...
      {
        $lookup: {
          // Join with the classes collection to get the price
          from: "classes",
          localField: "class",
          foreignField: "_id",
          as: "classDetails",
    ...
    const totalPaidBookings = await Booking.countDocuments({
      paymentStatus: "paid",
    });
