...
export const editUserInfo = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while editing the user profile" });
  }
};
...
export const deleteUserAccount = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while deleting your account." });
  }
};
...
export const bookClass = async (req, res) => {
  try {
    ...
    const classToBook = await Class.findById(classId).populate({
      path: "trainer",
...
export const submitFeedback = async (req, res) => {
  try {
    ...
    if (!booking) {
      return res
        .status(403)
        .json({ message: "You must book a class before you can review it." });
    }
    ...
    if (!trainer) {
      return res
        .status(404)
        .json({ message: "Trainer associated with this class not found" });
    }
...
export const updateFeedback = async (req, res) => {
  try {
    ...
    if (trainer && rating !== undefined) {
      const oldTotalScore =
        trainer.rating.averageRating * trainer.rating.totalReviews - oldRating;
      const newTotalScore = oldTotalScore + rating;
      trainer.rating.averageRating =
        newTotalScore / trainer.rating.totalReviews; // Total reviews stays the same
      await trainer.save();
    }
  ...
  try {
    ...
    if (trainer) {
      ...
      if (newTotalReviews > 0) {
        trainer.rating.averageRating =
          (oldTotalScore - review.rating) / newTotalReviews;
      } else {
...
export const allTrainers = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "error in the view all trainers controller" });
  }
};
...
export const viewTrainer = async (req, res) => {
  ...
  try {
    ...
    const classes = await Class.find({
      ...
      "classTitle classType duration timeSlot price classPic capacity attendees"
    );
